import { PlusIcon, SearchIcon, AlertTriangleIcon, PackageOpenIcon, MoreVerticalIcon, TrashIcon } from "lucide-react";
...
export const EntityPagination = ({ page, totalPages, onPageChange, disabled }: EntityPaginationProps) => {
  return (
    <div className="flex items-center justify-between gap-x-2 w-full">
      <div className="flex-1 text-sm text-muted-foreground" suppressHydrationWarning>
      ...
      <div className="flex items-center justify-end space-x-2 py-4">
        ...
        <Button disabled={page == totalPages || disabled} variant={'outline'} size={"sm"} onClick={() => onPageChange(Math.min(totalPages, page + 1))}>
          Next
        </Button>
      </div>
    </div>
...
export const EmptyView = ({ title, description, buttonLabel, onNew }: EmptyViewProps) => {
  return (
...
export const EntityItem = ({ href, title, subtitle, image, actions, onRemove, isRemoving, className }: EntityItemProps) => {
  ...
  return (
    <Link href={href} prefetch>
       <Card className={cn("p-4 shadow-none hover:shadow cursor-pointer transition-shadow", className, isRemoving ? "opacity-50 cursor-not-allowed" : "")}>
